...
async function run(){
  ...
  try{
    ...
    const caches=await win().caches.keys();ok(caches.includes('bubble-lab-v0.4.1'),'versioned PWA cache installed');const keys=await (await win().caches.open('bubble-lab-v0.4.1')).keys();ok(keys.length===8&&keys.every(r=>!new URL(r.url).pathname.includes('qa.')),'offline shell contains only eight static assets, no conversation data');ok(!caches.includes('bubble-lab-v0.4.0'),'previous application cache removed');
