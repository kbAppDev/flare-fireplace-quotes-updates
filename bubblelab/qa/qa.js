...
async function run(){
  ...
  try{
    await reset();ok($('appVersion').textContent==='0.4.2','version 0.4.2');
    ...
    const caches=await win().caches.keys();ok(caches.includes('bubble-lab-v0.4.2'),'versioned PWA cache installed');const keys=await (await win().caches.open('bubble-lab-v0.4.2')).keys();ok(keys.length===8&&keys.every(r=>!new URL(r.url).pathname.includes('qa.')),'offline shell contains only eight static assets, no conversation data');ok(!caches.includes('bubble-lab-v0.4.0'),'previous application cache removed');
