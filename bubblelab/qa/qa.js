...
async function run(){
  ...
  try{
    ...
    const target=hits()[0].dataset.id;click('composerHit');fill('messageText','Inline reply test');choose('messageReplyTo',target);click('saveMessageBtn');await wait(100);hits().at(-1).click();ok($('messageReplyTo').value===target,'inline reply persists');$('messageDeleted').checked=true;click('saveMessageBtn');await wait(100);hits().at(-1).click();ok($('messageDeleted').checked,'message removed flag persists');closeAll();
    ...
    const exported=await new Promise(resolve=>{const reader=new (win().FileReader)();reader.onload=()=>resolve(reader.result);reader.readAsDataURL(out);});ok(exported===before,'export equals visible canvas at scroll 170: no tools or browser chrome');closeAll();
