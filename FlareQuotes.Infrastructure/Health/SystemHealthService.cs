...
public sealed class SystemHealthService : ISystemHealthService
{
    ...
    public async Task<IReadOnlyList<SystemHealthItem>> CheckAsync(CancellationToken cancellationToken = default)
    {
        ...
        items.Add(CheckFile("Resource links workbook",
                            Path.Combine(AppContext.BaseDirectory, "LocalData", "resource_links.xlsx")));
        items.Add(CheckFile("Outdoor links workbook", Path.Combine(AppContext.BaseDirectory, "LocalData",
