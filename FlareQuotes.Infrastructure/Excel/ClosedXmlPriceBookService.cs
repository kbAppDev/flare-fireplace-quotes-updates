...
public sealed class ClosedXmlPriceBookService : IPriceBookService
{
    ...
    private static readonly Dictionary<string, decimal> LouverMsrpFallback = new(StringComparer.OrdinalIgnoreCase)
    {
    ...
    public Task<PriceBookWorkbook> LoadAsync(string path, CancellationToken cancellationToken = default)
    {
        ...
        foreach (var worksheet in workbook.Worksheets)
        {
            ...
            var headers = worksheet.Row(1).CellsUsed().Select((cell, index) => new { Name = cell.GetString(), Index = index + 1 }).ToList();
            foreach (var row in worksheet.RowsUsed().Skip(1))
            {
                ...
                if (!string.IsNullOrWhiteSpace(priceRow.Sku) ||
                    !string.IsNullOrWhiteSpace(priceRow.PartName) ||
    ...
    public async Task<PriceBookMatch> FindBaseModelAsync(QuoteRequest request, CancellationToken cancellationToken = default)
    {
        ...
        return first?.BaseLine.Price is null
            ...
            : new PriceBookMatch { Found = true, Row = new PriceRow { Sku = first.BaseLine.Sku, Description = first.BaseLine.Description, Price = first.BaseLine.Price }, Reason = "Matched" };
    }

    public async Task<PriceBookMatch> FindFeaturePriceAsync(QuoteRequest request, FeatureOption feature, CancellationToken cancellationToken = default)
    {
        var workbook = await LoadAsync(GetDefaultPricingPath(), cancellationToken);
        var row = FindFeatureRow(workbook, DetectType(request.Model, request.Size), request.Model, request.Size, request.GlassHeight, feature.DisplayName);
        return row is null
            ? new PriceBookMatch { Found = false, Reason = $"No row found for {feature.DisplayName}" }
            : new PriceBookMatch { Found = true, Row = row, Reason = "Matched" };
    }

    public async Task<PricedQuoteResult> BuildPricedQuoteAsync(QuoteRequest request, string pricingPath, CancellationToken cancellationToken = default)
    {
        var workbook = await LoadAsync(pricingPath, cancellationToken);
        if (workbook.Rows.Count == 0)
            return new PricedQuoteResult { Success = false, Request = request, Message = $"Pricing file not found or empty: {pricingPath}" };

        var result = new PricedQuoteResult { Request = request, Success = true };
        IEnumerable<FireplaceQuote> fireplaceInputs = request.Fireplaces.Count > 0 ? request.Fireplaces : new List<FireplaceQuote> { ToFireplace(request) };

        foreach (var input in fireplaceInputs)
        {
                        // v1.4.1 block invalid Large See Through resource links
            if (IsInvalidLargeSeeThroughModel(input.Model, input.Size))
                continue;
var type = input.Type == FireplaceType.Unknown ? DetectType(input.Model, input.Size) : input.Type;
            // v1.4.2 Outdoor Kit type promotion
            type = ApplyIndoorOutdoorSeeThroughForOutdoorKit(type, input.Model, input.Features);
            var baseRow = FindBaseRow(workbook, type, input.Model, input.Size, input.GlassHeight);
                        // v1.4.2 OD/IO included outdoor kit row
            var includedOutdoorKitRow = FindIncludedOutdoorKitRow(workbook, type, input.Model, input.Size, input.GlassHeight);
var modelNumber = ResolveModelNumber(workbook, type, input.Model, input.Size, input.GlassHeight) ?? BuildModelNumber(type, input.Model, input.Size, input.GlassHeight);
            var priced = new PricedFireplaceQuote
            {
                ...
                Model = input.Model,
                Size = input.Size,
                GlassHeight = input.GlassHeight,
                ModelNumber = modelNumber,
                Description = baseRow?.Description ?? BuildDescription(type, input.Model, input.Size, input.GlassHeight),
                LeadTime = string.IsNullOrWhiteSpace(input.LeadTime) ? "3-5 Business Days" : input.LeadTime,
                ClassicMediaDisplay = !string.IsNullOrWhiteSpace(input.ClassicMediaDisplay) ? input.ClassicMediaDisplay : (request.ClassicMedia.FirstOrDefault()?.DisplayName ?? string.Empty),
            ...
            {
                                // v1.4.2 skip manual Outdoor Kit optional feature
                if (IsOutdoorKitFeature($"{feature.Key} {feature.DisplayName} {feature.PdfDescription}"))
                    continue;
if (IsDoubleCorner(input.Model, input.Size, input.GlassHeight) && IsReflectiveSidesFeature(feature.DisplayName))
                    continue;

                if (IsRoomDefiner(input.Model, input.Size, input.GlassHeight) && IsRoomDefinerReflectiveFeature(feature.DisplayName))
                    continue;

                var row = FindFeatureRow(workbook, type, input.Model, input.Size, input.GlassHeight, feature.DisplayName);
                var featureDescription = OverrideFeatureDescription(feature.DisplayName, row?.Description ?? feature.PdfDescription);
                ...
                if (type is FireplaceType.Outdoor or FireplaceType.OutdoorSeeThrough)
                {
                    ...
                    if (normalizedFeature.Contains("safety screen"))
                        featureDescription = OutdoorVentFreeScreenDescription(type, input.Model);
                }
            ...
            foreach (var media in input.PremiumMedia)
            {
                ...
                {
                    priced.OptionalFeatures.Add(BuildStoneBallsPriceLine(media, input.Size));
                    continue;
                }

                if (IsDriftwoodMedia(media))
                {
                    var driftwoodLine = BuildDriftwoodPriceLine(workbook, type, input.Size, input.Model);
                    if (!media.IsPremium)
                    {
                        driftwoodLine.Feature = "Add. Classic Media - Driftwood";
                        driftwoodLine.Description = driftwoodLine.Description.Replace("Driftwood Logs", "Add. Classic Media - Driftwood Logs", StringComparison.OrdinalIgnoreCase);
                    }
                ...
                var quantity = CalculatePremiumMediaQuantity(workbook, type, media, input.Size, input.Model);
                var mediaFeatureName = media.IsPremium ? media.DisplayName : $"Add. Classic Media - {media.DisplayName}";
                priced.OptionalFeatures.Add(new PriceLine
                {
                    Feature = mediaFeatureName,
                    Description = quantity > 1 ? $"{mediaFeatureName} - {quantity} sets" : mediaFeatureName,
                    Sku = row?.Sku ?? string.Empty,
                    Quantity = quantity,
        ...
        if (string.IsNullOrWhiteSpace(result.Message)) result.Message = result.Success ? "Pricing complete." : "Pricing completed with missing rows.";
        return result;
    }

    public async Task<IReadOnlyList<ResourceLinkSet>> ResolveResourceLinksAsync(QuoteRequest request, string pricingPath, CancellationToken cancellationToken = default)
    {
...
var type = input.Type == FireplaceType.Unknown ? DetectType(input.Model, input.Size) : input.Type;
            // v1.4.2 Outdoor Kit type promotion
            type = ApplyIndoorOutdoorSeeThroughForOutdoorKit(type, input.Model, input.Features);
            var linkRow = TryGetOutdoorVentFreeResourceRowFromWorkbook(type, input.Model, input.Size, input.GlassHeight)
                ?? TryGetLargeResourceRow(type, input.Model, input.Size, input.GlassHeight)
                ?? FindResourceRow(workbook, type, input.Model, input.Size, input.GlassHeight);
                        // v1.4.1 Requested resource input model marker
            if (linkRow is not null)
                linkRow.RawValues["Requested Model"] =
                    type == FireplaceType.IndoorOutdoorSeeThrough ||
                    IsIndoorOutdoorSeeThroughModelCode(input.Model) ||
                    HasOutdoorKitFeature(input.Features)
                        ? $"{input.Model} indoor outdoor outdoor kit"
                        : input.Model ?? string.Empty;
var modelNumber = linkRow is not null
                ? First(linkRow.RawValues, "Model #", "Model Number", "Model")
                : BuildModelNumber(type, input.Model, input.Size, input.GlassHeight);

            if (string.IsNullOrWhiteSpace(modelNumber))
                modelNumber = BuildModelNumber(type, input.Model, input.Size, input.GlassHeight);

                        // v1.4.1 Passage resource model-number override
            if (IsPassageModel(input.Model))
                modelNumber = PassageModelCode(input.Model);
if (!seen.Add(modelNumber))
                continue;

            var set = new ResourceLinkSet { ModelNumber = modelNumber };
            var columns = ResourceColumns(type);
            var rowFallback = linkRow is not null
                ? CleanCellUrl(First(linkRow.RawValues, "Fallback URL", "Fallback", "Download Center", "Download Center URL"))
                : string.Empty;
    ...
    private async Task<PriceBookWorkbook> LoadResourceWorkbookAsync(string pricingPath, CancellationToken cancellationToken)
    {
    ...
    private static IEnumerable<string> ResourceWorkbookCandidates(string pricingPath)
    {
        var names = new[]
        {
            ...
            "Resource Links.xlsx",
            "shared_pricing.xlsx",
            "pricing_with_resource_links.xlsx",
            "pricing-resource-links.xlsx"
        };

        var directories = new List<string>();

        var pricingDir = string.IsNullOrWhiteSpace(pricingPath) ? string.Empty : Path.GetDirectoryName(Path.GetFullPath(pricingPath)) ?? string.Empty;
        if (!string.IsNullOrWhiteSpace(pricingDir)) directories.Add(pricingDir);
        ...
        directories.Add(Path.Combine(Environment.GetFolderPath(Environment.SpecialFolder.LocalApplicationData), "Flare Fireplaces - Quotes", "v3"));
        directories.Add(Path.Combine(Environment.GetFolderPath(Environment.SpecialFolder.LocalApplicationData), "Flare Quote Builder"));
    ...
    {
        const string sharedPricingExportUrl = "https://docs.google.com/spreadsheets/d/1kBfDyekOABQckF22v1mXzk59apccLBI1GCWi2CHO9zA/export?format=xlsx";

        try
        {
            var cacheDir = Path.Combine(Environment.GetFolderPath(Environment.SpecialFolder.LocalApplicationData), "Flare Fireplaces - Quotes", "v3");
    ...
    private static bool HasOutdoorKitFeature(IEnumerable<FeatureSelection>? features)
    {
        return features?.Any(feature =>
            IsOutdoorKitFeature($"{feature.Key} {feature.DisplayName} {feature.PdfDescription}")) == true;
    }

    private static FireplaceType ApplyIndoorOutdoorSeeThroughForOutdoorKit(FireplaceType type, string? model, IEnumerable<FeatureSelection>? features)
    {
                // v1.4.2 ApplyIndoorOutdoorSeeThroughForOutdoorKit OD/IO model code
        if (IsIndoorOutdoorSeeThroughModelCode(model))
            return FireplaceType.IndoorOutdoorSeeThrough;
if (HasOutdoorKitFeature(features) &&
            IsSeeThroughForReflectiveBackRules(type, model) &&
    ...
    private static FireplaceQuote ToFireplace(QuoteRequest request) => new()
    {
        ...
        Type = ApplyIndoorOutdoorSeeThroughForOutdoorKit(DetectType(request.Model, request.Size), request.Model, request.SelectedFeatures),
        Model = request.Model,
        Size = request.Size,
    ...
    private static PriceRow? FindExactLargeBaseRow(IEnumerable<PriceRow> rows, FireplaceType type, string model, string size, string glassHeight)
    {
        ...
        var expectedPartName = string.IsNullOrWhiteSpace(suffix)
            ? $"FLARE-{style}-{sizeNum}"
            : $"FLARE-{style}-{sizeNum}-{suffix}";
...
private static PriceRow? FindBaseRow(PriceBookWorkbook wb, FireplaceType type, string model, string size, string glassHeight)
    {
                // v1.4.1 block invalid Large See Through base row
...
type = ForceOutdoorVentFreeTypeFromModel(type, model);
        ...
        var glassNum = FirstNonBlank(GlassInches(glassHeight), GlassInches(ExtractGlassHeightFromModelCode(model)));
                ...
                // v1.4.0 Passage base-row override
...
if (type is FireplaceType.Outdoor or FireplaceType.OutdoorSeeThrough)
        {
            ...
            var part = string.IsNullOrWhiteSpace(suffix)
                ? $"FLARE-{vfCode}-{sizeNum}"
                : $"FLARE-{vfCode}-{sizeNum}-{suffix}";
            ...
            return Best(rows, r =>
                Text(r).Contains("vent free", StringComparison.OrdinalIgnoreCase) &&
                ContainsAll(Text(r), style) &&
        ...
        if (type == FireplaceType.Traditional)
            return Best(rows, r =>
                Text(r).Contains("traditional", StringComparison.OrdinalIgnoreCase) &&
                ContainsSize(r, sizeNum) &&
...
private static PriceRow? FindFeatureRow(PriceBookWorkbook wb, FireplaceType type, string model, string size, string glassHeight, string feature)
    {
                // v1.4.1 block invalid Large See Through feature row
        if (IsInvalidLargeSeeThroughModel(model, size))
            return null;
glassHeight = EffectiveGlassHeight(glassHeight, model);
        ...
        var styleWords = StyleWords(type, model);
                ...
                // v1.4.0 Passage feature-row override
        if (IsPassageModel(model))
        {
...
if (f.Contains("power vent"))
            return Best(rows, r => Eq(r.SheetName, "Indoor Price Book") && Text(r).Contains("power vent fan", StringComparison.OrdinalIgnoreCase) && !Text(r).Contains("cable", StringComparison.OrdinalIgnoreCase));
        ...
        if (f.Contains("summer kit"))
        {
            if (type == FireplaceType.Large)
            {
                ...
                return Best(rows, r => Eq(r.SheetName, "Large Price Book") && Text(r).Contains($"SK-Lrg-{kits}", StringComparison.OrdinalIgnoreCase) && !Text(r).Contains("EOL", StringComparison.OrdinalIgnoreCase));
            }
            return Best(rows, r => Eq(r.SheetName, "Indoor Price Book") && Text(r).Contains("summer kit fan", StringComparison.OrdinalIgnoreCase) && !Text(r).Contains("End-of-Line", StringComparison.OrdinalIgnoreCase) && !Text(r).Contains("EOL", StringComparison.OrdinalIgnoreCase));
        }

        if (f.Contains("reflective black interior"))
            return Best(rows, r => Eq(r.SheetName, "Indoor Price Book") && (Text(r).Contains($"RB-TRA-{sizeNum}", StringComparison.OrdinalIgnoreCase) || Text(r).Contains($"RBTRA{sizeNum}", StringComparison.OrdinalIgnoreCase)));

        if (f.Contains("double glass"))
        {
            if (type == FireplaceType.Traditional)
                return Best(rows, r => Eq(r.SheetName, "Indoor Price Book") && Text(r).Contains($"CLG-TRA-{sizeNum}", StringComparison.OrdinalIgnoreCase));
            return Best(rows, r => Eq(r.SheetName, "Indoor Price Book") && Text(r).Contains("double glass", StringComparison.OrdinalIgnoreCase) && Text(r).Contains(style, StringComparison.OrdinalIgnoreCase) && ContainsSize(r, sizeNum) && (string.IsNullOrWhiteSpace(glassNum) || ContainsGlass(r, glassNum)));
        }

        if (f.Contains("summit"))
            return Best(rows, r => Eq(r.SheetName, "Indoor Price Book") && Text(r).Contains("summit burner", StringComparison.OrdinalIgnoreCase) && ContainsSize(r, sizeNum));

        if (f.Contains("rgb"))
            return Best(rows, r => Eq(r.SheetName, type == FireplaceType.Large ? "Large Price Book" : type is FireplaceType.Outdoor or FireplaceType.OutdoorSeeThrough ? "Outdoor Price Book" : "Indoor Price Book") && Text(r).Contains("rgb", StringComparison.OrdinalIgnoreCase) && (ContainsSize(r, sizeNum) || type is FireplaceType.Outdoor or FireplaceType.OutdoorSeeThrough));

        if (f.Contains("reflective black sides"))
            return FindReflectiveSidesRow(rows, type, model, glassHeight);

        
        // v1.4.2 Outdoor Kit FindFeatureRow
        if (IsOutdoorKitFeature(f))
            return FindOutdoorKitRow(rows, type, model, sizeNum, glassHeight);if (f.Contains("reflective black back") || f.Contains("reflective back"))
        {
    ...
    private static PriceRow? FindIncludedOutdoorKitRow(PriceBookWorkbook wb, FireplaceType type, string model, string size, string glassHeight)
    {
                // v1.4.2 style-only STOD/STIO outdoor kit lookup
        if (IsIndoorOutdoorSeeThroughModelCode(model) && !IsPassageModel(model))
        {
            ...
            var odSkuSuffixForIncludedKit = odSuffixForIncludedKit.Equals("EH", StringComparison.OrdinalIgnoreCase) ? "E" : odSuffixForIncludedKit;
            ...
            var odExactIncludedKitRow = Best(odRowsForIncludedKit, r =>
            {
                ...
                return text.Contains("outdoor kit", StringComparison.OrdinalIgnoreCase) &&
                       (compactText.Contains(odExactPartCompactForIncludedKit, StringComparison.OrdinalIgnoreCase) ||
...
if (type != FireplaceType.IndoorOutdoorSeeThrough && !IsIndoorOutdoorSeeThroughModelCode(model))
        ...
        if (IsPassageModel(model) ||
            Regex.IsMatch(compactModel, @"^(STPASS|PASSST)(OD|IO)?$", RegexOptions.IgnoreCase))
        {
            return Best(rows, r =>
            {
                if (!Eq(r.SheetName, "Indoor Price Book"))
                ...
                return text.Contains("outdoor kit", StringComparison.OrdinalIgnoreCase) &&
                       ...
                       (compactText.Contains("ODSTPASS", StringComparison.OrdinalIgnoreCase) ||
                        ...
                        compactText.Contains("OUTDOORKITFORSEETHROUGHPASSAGE", StringComparison.OrdinalIgnoreCase));
            });
        }
        ...
        var exactPart = string.IsNullOrWhiteSpace(suffix)
            ? $"OD-ST-{sizeNum}"
            : $"OD-ST-{sizeNum}-{suffix}";
...
private static PriceRow? FindOutdoorKitRow(IReadOnlyList<PriceRow> rows, FireplaceType type, string model, string sizeNum, string glassHeight)
    {
        ...
        {
            return Best(rows, r =>
                Eq(r.SheetName, "Indoor Price Book") &&
    ...
    private static PriceRow? FindTraditionalBrickRow(IReadOnlyList<PriceRow> rows, string feature, string sizeNum)
    {
        ...
        return Best(rows, r =>
        {
            if (!Eq(r.SheetName, "Indoor Price Book")) return false;
            var text = Text(r);
            if (!text.Contains("Brick", StringComparison.OrdinalIgnoreCase)) return false;
            if (!ContainsSize(r, sizeNum)) return false;

            var isHerringbone = f.Contains("herringbone");
            if (isHerringbone && !text.Contains("Herringbone", StringComparison.OrdinalIgnoreCase)) return false;
            if (!isHerringbone && text.Contains("Herringbone", StringComparison.OrdinalIgnoreCase)) return false;

            if (f.Contains("light stone")) return text.Contains("Light Stone", StringComparison.OrdinalIgnoreCase);
            if (f.Contains("natural")) return text.Contains("Natural", StringComparison.OrdinalIgnoreCase);
            if (f.Contains("black")) return text.Contains("Black", StringComparison.OrdinalIgnoreCase);
            if (f.Contains("red")) return text.Contains("Red", StringComparison.OrdinalIgnoreCase);
            return false;
        });
    }

    private static PriceRow? FindPassiveHeatFlexRow(IReadOnlyList<PriceRow> rows, FireplaceType type, string model, string sizeNum)
    {
    ...
    private static PriceRow? FindActiveHeatFlexRow(IReadOnlyList<PriceRow> rows, FireplaceType type, string model, string sizeNum)
    {
    ...
    private static string VentFreeModelStyleCode(FireplaceType type, string model)
    {
        ...
        if (type == FireplaceType.OutdoorSeeThrough ||
            normalized.Contains("see through") ||
            normalized.Contains("see-through") ||
            normalized.Contains("st"))
            return "VST";

        if (normalized.Contains("left") || normalized.Contains("lc")) return "VLC";
        if (normalized.Contains("right") || normalized.Contains("rc")) return "VRC";
        if (normalized.Contains("double") || normalized.Contains("dc")) return "VDC";
    ...
    private static PriceRow? FindOutdoorSafetyScreenRow(IReadOnlyList<PriceRow> rows, FireplaceType type, string model, string sizeNum, string glassHeight)
    {
        if (string.IsNullOrWhiteSpace(sizeNum))
            return null;
        ...
        var fallback = Best(rows, r =>
            Eq(r.SheetName, "Outdoor Price Book") &&
            Text(r).Contains("safety", StringComparison.OrdinalIgnoreCase) &&
            Text(r).Contains("screen", StringComparison.OrdinalIgnoreCase) &&
            ContainsSize(r, sizeNum) &&
...
private static PriceRow? FindReflectiveBackRow(IReadOnlyList<PriceRow> rows, FireplaceType type, string model, string sizeNum, string glassHeight)
    {
        // See Through fireplaces do not have a back; use Reflective Sides instead where applicable.
        if (IsSeeThroughForReflectiveBackRules(type, model))
            return null;
        var sheet = type is FireplaceType.Outdoor or FireplaceType.OutdoorSeeThrough ? "Outdoor Price Book" : "Indoor Price Book";
        var style = StyleCode(type, model);
        ...
        var exact = string.IsNullOrWhiteSpace(glassSuffix)
            ? $"{prefix}-{style}-{sizeNum}"
            : $"{prefix}-{style}-{sizeNum}-{glassSuffix}";

        var exactNoDash = exact.Replace("-", string.Empty);
        var bySku = Best(rows, r => Eq(r.SheetName, sheet) &&
        ...
        return Best(rows, r => Eq(r.SheetName, sheet) &&
            ...
            ContainsAll(Text(r), StyleWords(type, model)) &&
            ContainsSize(r, sizeNum) &&
            (string.IsNullOrWhiteSpace(GlassInches(glassHeight)) || ContainsGlass(r, GlassInches(glassHeight))));
    }

    private static PriceRow? FindReflectiveSidesRow(IReadOnlyList<PriceRow> rows, FireplaceType type, string model, string glassHeight)
    {
        var sheet = type == FireplaceType.Large ? "Large Price Book" : type is FireplaceType.Outdoor or FireplaceType.OutdoorSeeThrough ? "Outdoor Price Book" : "Indoor Price Book";
        var style = StyleCode(type, model);
        var suffix = GlassSuffix(glassHeight);
        ...
        var exact = string.IsNullOrWhiteSpace(suffix)
            ? $"{prefix}-{styleForSku}"
            : $"{prefix}-{styleForSku}-{suffix}";
    ...
    private static PriceRow? FindLouverRowBySize(PriceBookWorkbook wb, string size)
    {
        ...
        var exact = Best(rows, r =>
            Eq((r.PartName ?? string.Empty).Replace(" ", string.Empty), exactPart) ||
            Eq((r.Sku ?? string.Empty).Replace(" ", string.Empty), exactSku));
        if (exact is not null)
            return exact;

        var byDescription = Best(rows, r =>
            Text(r).Contains("louver", StringComparison.OrdinalIgnoreCase) &&
    ...
    private static PriceRow? FindPremiumMediaRow(PriceBookWorkbook wb, string key, string display)
    {
        ...
        if (norm.Contains("black fire glass")) return BySku("CMDFGBL", "MEDIA-FG-Black");
        if (norm.Contains("reflective silver") || norm.Contains("silver")) return BySku("CMDFGS", "MEDIA-FG-Silver");
        if (norm.Contains("bronze")) return BySku("CMDFGBR", "MEDIA-FG-Bronze");
        if (norm.Contains("ice")) return BySku("CMDFGIC", "MEDIA-FG-Ice");
        if (norm.Contains("clear") && norm.Contains("glass")) return BySku("CMDFGCL", "MEDIA-FG-Clear");
        if (norm.Contains("black diamond") || norm.Contains("zircon")) return BySku("CMDPDB", "MEDIA-PD-Black");
        if (norm.Contains("rain")) return BySku("CMDPDR", "MEDIA-PD-Rain");
        if (norm.Contains("white") && norm.Contains("pebble")) return BySku("CMDCPW", "MEDIA-CP-White");
        if (norm.Contains("black") && norm.Contains("pebble")) return BySku("CMDCPB", "MEDIA-CP-Black");
        if (norm.Contains("earth") && norm.Contains("pebble")) return BySku("CMDCPE", "MEDIA-CP-Earth");
        if (norm.Contains("white") && norm.Contains("log")) return BySku("CMDGLW", "MEDIA-GL-White");
        if (norm.Contains("ceramic logs") || norm.Contains("wood logs")) return BySku("CMDGLWO", "MEDIA-GL-Wood");
        if (norm.Contains("branch")) return BySku("CMDGLB", "MEDIA-GL-Branches");
        if (norm.Contains("black") && norm.Contains("ember")) return BySku("CMDGLEB", "MEDIA-GL-Embers-Bla");
        if (norm.Contains("ember")) return BySku("CMDGLE", "MEDIA-GL-Embers");
        if (norm.Contains("tr42bch") || norm.Contains("tr42 birch") || (norm.Contains("traditional 42") && norm.Contains("birch")))
            return BySku("TR42BCH", "TR42-BIRCH");

        if (norm.Contains("tr46bch") || norm.Contains("tr46 birch") || (norm.Contains("traditional 46") && norm.Contains("birch")))
        ...
        if (norm.Contains("gold") && norm.Contains("glass")) return Best(rows, r => Text(r).Contains("Gold", StringComparison.OrdinalIgnoreCase) && Text(r).Contains("Glass", StringComparison.OrdinalIgnoreCase));
        if (norm.Contains("aqua") && norm.Contains("glass")) return Best(rows, r => Text(r).Contains("Aqua", StringComparison.OrdinalIgnoreCase) && Text(r).Contains("Glass", StringComparison.OrdinalIgnoreCase));
        if (norm.Contains("black stones") || norm.Contains("premium black")) return Best(rows, r => Text(r).Contains("Premium Black", StringComparison.OrdinalIgnoreCase));
        if (norm.Contains("white stones") || norm.Contains("cottage white stones")) return Best(rows, r => Text(r).Contains("Cottage White", StringComparison.OrdinalIgnoreCase) && Text(r).Contains("Stones", StringComparison.OrdinalIgnoreCase));
        if (norm.Contains("birch")) return Best(rows, r => Text(r).Contains("Birchwood", StringComparison.OrdinalIgnoreCase) || Text(r).Contains("PMDBIRCH", StringComparison.OrdinalIgnoreCase) || Text(r).Contains("PSA-Birch", StringComparison.OrdinalIgnoreCase));
        if (norm.Equals("driftwood", StringComparison.OrdinalIgnoreCase) || norm.Contains("premium driftwood"))
            return Best(rows, r => Text(r).Contains("Large Drift", StringComparison.OrdinalIgnoreCase)) ?? Best(rows, r => Text(r).Contains("Small Drift", StringComparison.OrdinalIgnoreCase));
        if (norm.Contains("small drift")) return Best(rows, r => Text(r).Contains("Small Drift", StringComparison.OrdinalIgnoreCase));
        if (norm.Contains("large drift")) return Best(rows, r => Text(r).Contains("Large Drift", StringComparison.OrdinalIgnoreCase));
        if (norm.Contains("grey") && norm.Contains("2")) return Best(rows, r => Text(r).Contains("Cape Grey", StringComparison.OrdinalIgnoreCase) && Text(r).Contains("2\"", StringComparison.OrdinalIgnoreCase));
        if (norm.Contains("grey") && norm.Contains("4")) return Best(rows, r => Text(r).Contains("Cape Grey", StringComparison.OrdinalIgnoreCase) && Text(r).Contains("4\"", StringComparison.OrdinalIgnoreCase));
        if (norm.Contains("grey")) return Best(rows, r => Text(r).Contains("Cape Grey", StringComparison.OrdinalIgnoreCase) && Text(r).Contains("Mixed", StringComparison.OrdinalIgnoreCase));
        if (norm.Contains("black") && norm.Contains("2")) return Best(rows, r => Text(r).Contains("Matte Black", StringComparison.OrdinalIgnoreCase) && Text(r).Contains("2\"", StringComparison.OrdinalIgnoreCase));
        if (norm.Contains("black") && norm.Contains("4")) return Best(rows, r => Text(r).Contains("Matte Black", StringComparison.OrdinalIgnoreCase) && Text(r).Contains("4\"", StringComparison.OrdinalIgnoreCase));
        if (norm.Contains("black")) return Best(rows, r => Text(r).Contains("Matte Black", StringComparison.OrdinalIgnoreCase) && Text(r).Contains("Mixed", StringComparison.OrdinalIgnoreCase));
        if (norm.Contains("white") && norm.Contains("2")) return Best(rows, r => Text(r).Contains("Cottage White", StringComparison.OrdinalIgnoreCase) && Text(r).Contains("2\"", StringComparison.OrdinalIgnoreCase));
        if (norm.Contains("white") && norm.Contains("4")) return Best(rows, r => Text(r).Contains("Cottage White", StringComparison.OrdinalIgnoreCase) && Text(r).Contains("4\"", StringComparison.OrdinalIgnoreCase));
        if (norm.Contains("white")) return Best(rows, r => Text(r).Contains("Cottage White", StringComparison.OrdinalIgnoreCase) && Text(r).Contains("Mixed", StringComparison.OrdinalIgnoreCase));
    ...
    private static PriceRow? TryGetOutdoorVentFreeResourceRowFromWorkbook(FireplaceType type, string model, string size, string glassHeight)
    {
        ...
        try
        {
            ...
            var candidates = new[]
            {
                ...
                Path.Combine(Environment.GetFolderPath(Environment.SpecialFolder.UserProfile), "OneDrive", "Desktop", "Quote Request", "LocalData", "outdoor_spec_center_extracted_links.xlsx"),
                Path.Combine(Environment.GetFolderPath(Environment.SpecialFolder.UserProfile), "OneDrive", "Desktop", "Flare Quotes", "LocalData", "outdoor_spec_center_extracted_links.xlsx")
            };

            var workbookPath = candidates
            ...
            var sheet = workbook.Worksheets.FirstOrDefault(ws =>
                ws.Name.Equals("App Resource Rows", StringComparison.OrdinalIgnoreCase));
            ...
            var dataRows = hasHeader
                ? usedRows.Where(row => row.RowNumber() > headerRow!.RowNumber())
                : usedRows;
            ...
            foreach (var row in dataRows)
            {
                ...
                var rowKeys = new[]
                {
                    NormalizeOutdoorResourceKey(modelKey),
                    NormalizeOutdoorResourceKey(compactKey)
                }
    ...
    private static IEnumerable<string> OutdoorVentFreeResourceKeys(FireplaceType type, string model, string size, string glassHeight)
    {
        var digits = Digits(size);
        var suffix = GlassSuffix(glassHeight);
                // v1.4.1 Outdoor resource size/glass recovery
...
var style = VentFreeModelStyleCode(type, model);
    ...
    {
                // v1.4.1 block invalid Large See Through large resource row
        if (IsInvalidLargeSeeThroughModel(model, size))
            return null;
if (type != FireplaceType.Large)
            return null;
        ...
        var modelDash = string.IsNullOrWhiteSpace(suffix)
            ? $"{style}-{digits}"
            : $"{style}-{digits}-{suffix}";
    ...
    private static string LargeResourceStyleCode(string model)
    {
        ...
        if (compact.StartsWith("ST") || normalized.Contains("see through") || normalized.Contains("see-through")) return "ST";
        if (compact.StartsWith("LC") || normalized.Contains("left corner")) return "LC";
        if (compact.StartsWith("RC") || normalized.Contains("right corner")) return "RC";
        if (compact.StartsWith("DC") || normalized.Contains("double corner")) return "DC";
        return "FF";
    }
...
private static PriceRow? FindResourceRow(PriceBookWorkbook wb, FireplaceType type, string model, string size, string glassHeight)
    {
        glassHeight = EffectiveGlassHeight(glassHeight, model);
        ...
        if (scopedRows.Count == 0)
            scopedRows = rows;
                ...
                // v1.4.1 Passage resource row scoping
        if (IsPassageModel(model))
        {
...
var candidates = ResourceModelCandidates(type, model, size, glassHeight)
        ...
        var styleSizeMatches = scopedRows
            .Where(r => ContainsAll(Text(r), style) && ContainsSize(r, sizeNum))
            .ToList();

        if (type != FireplaceType.Traditional && !string.IsNullOrWhiteSpace(glassNum))
        {
            var glassSpecific = styleSizeMatches.FirstOrDefault(r =>
                Eq(First(r.RawValues, "Glass Height", "Glass", "Glass Ht"), glassNum) ||
                ContainsGlass(r, glassNum) ||
                NormalizeResourceModelKey(ResourceModelKey(r)).EndsWith(GlassSuffix(glassHeight), StringComparison.OrdinalIgnoreCase));
    ...
    private static string? ResolveModelNumber(PriceBookWorkbook wb, FireplaceType type, string model, string size, string glassHeight) =>
        First(FindResourceRow(wb, type, model, size, glassHeight)?.RawValues ?? new Dictionary<string, string>(), "Model #", "Model Number", "Model");

    private static bool IsResourceLinksSheet(string sheetName) =>
        sheetName.Contains("Resource", StringComparison.OrdinalIgnoreCase) && sheetName.Contains("Link", StringComparison.OrdinalIgnoreCase);
    ...
    private static bool ResourceTemplateMatches(PriceRow row, string template)
    {
        ...
        return normalizedRow.Contains(normalizedTemplate, StringComparison.OrdinalIgnoreCase) || normalizedTemplate.Contains(normalizedRow, StringComparison.OrdinalIgnoreCase);
    }
    ...
    private static bool ResourceRowMatchesRequestedType(PriceRow row, FireplaceType type, string template)
    {
        ...
        var isVentFreeOutdoorKey =
            modelKey.StartsWith("VFF", StringComparison.OrdinalIgnoreCase) ||
        ...
        var saysOutdoor = isVentFreeOutdoorKey || isOutdoorSuffixKey || rowTemplate.Contains("outdoor", StringComparison.OrdinalIgnoreCase) || rowText.Contains("outdoor", StringComparison.OrdinalIgnoreCase) || rowText.Contains("vent free", StringComparison.OrdinalIgnoreCase);
        var saysIndoor = rowTemplate.Contains("indoor", StringComparison.OrdinalIgnoreCase) || rowText.Contains("indoor", StringComparison.OrdinalIgnoreCase);
        var saysTraditional = rowTemplate.Contains("traditional", StringComparison.OrdinalIgnoreCase) || rowText.Contains("traditional", StringComparison.OrdinalIgnoreCase) || modelKey.Contains("TRA", StringComparison.OrdinalIgnoreCase);
        var saysLarge = rowTemplate.Contains("large", StringComparison.OrdinalIgnoreCase) || rowText.Contains("large", StringComparison.OrdinalIgnoreCase);
    ...
    private static IEnumerable<string> ResourceModelCandidates(FireplaceType type, string model, string size, string glassHeight)
    {
        var digits = Digits(size);
        var suffix = GlassSuffix(glassHeight);
        ...
        if (IsPassageModel(model) && string.IsNullOrWhiteSpace(digits))
            digits = "30";
        // v1.4.0 Passage resource candidates
        if (IsPassageModel(model))
        {
...
private static string ResourceModelKey(PriceRow row) =>
        First(row.RawValues, "Model #", "Model Number", "Model", "SKU", "Part #", "Part Number");

    
    private static bool IsPassageResourceRow(PriceRow row)
    {
        var text = $"{ResourceModelKey(row)} {Text(row)} {First(row.RawValues, "Style", "Fireplace Style", "Description", "Template", "Category", "Type")}";
...
private static string NormalizeResourceModelKey(string value)
    ...
    private static string ResourceUrl(PriceRow row, string label)
    {
        ...
        if (IsPassageResourceRow(row))
        {
            ...
            var compact = Compact(text);            var requestedModel = First(row.RawValues, "Requested Model", "RequestedModel", "Input Model", "InputModel");
            var requestedCompact = Compact(requestedModel);
            var requestedNormalized = Normalize(requestedModel ?? string.Empty);

            var isOutdoorPassage =
                requestedCompact.Contains("OD", StringComparison.OrdinalIgnoreCase) ||
        ...
        if (requestedLabel == "Product Sheet") url = CleanCellUrl(First(row.RawValues, "Product", "Product Spec", "Product Specs"));
        if (requestedLabel == "Framing Guide") url = CleanCellUrl(First(row.RawValues, "Wood Framing", "Framing", "Framing Guide"));
        if (requestedLabel == "Dimension File") url = CleanCellUrl(First(row.RawValues, "Dimensions", "Dimension", "Dimension File"));
        if (requestedLabel == "3-Part Spec") url = CleanCellUrl(First(row.RawValues, "3 Part Spec", "Three Part Spec", "3-Part Spec"));
    ...
    private static string CleanCellUrl(string value)
    {
        ...
        return text.StartsWith("http://", StringComparison.OrdinalIgnoreCase) || text.StartsWith("https://", StringComparison.OrdinalIgnoreCase) ? text : string.Empty;
    }

    private static string[] ResourceColumns(FireplaceType type) => type switch
    {
        FireplaceType.Outdoor or FireplaceType.OutdoorSeeThrough or FireplaceType.IndoorOutdoorSeeThrough => ["Product Sheet", "Framing Guide", "Dimension File", "CAD", "SketchUp", "Revit"],
    ...
    private static string FallbackUrl(FireplaceType type) => type is FireplaceType.Outdoor or FireplaceType.OutdoorSeeThrough or FireplaceType.IndoorOutdoorSeeThrough
    ...
    private static string CanonicalFeatureName(string feature)
    {
        ...
        if (f.Contains("summit burner")) return "Summit Burner";
        if (f.Contains("double glass")) return "Double Glass";
        if (f.Contains("reflective black back")) return "Reflective Black Back";
        if (f.Contains("reflective black sides")) return "Reflective Black Sides";
        if (f.Contains("rgb")) return "RGB LEDs";
        if (f.Contains("summer kit")) return "Summer Kit";
        if (f.Contains("active heat flex")) return "Active Heat Flex";
        if (f.Contains("passive heat flex")) return "Passive Heat Flex";
        if (f.Contains("heat release louver")) return "Heat Release Louver";
        if (f.Contains("air intake louver")) return "Air Intake Louver";
        if (f.Contains("power vent")) return "Power Vent";
        if (f.Contains("reflective black interior")) return "Reflective Black Interior";
        if (f.Contains("brick traditional")) return feature.Trim();
    ...
    private static string OverrideFeatureDescription(string feature, string desc)
    {
        ...
        if (f.Contains("summit burner")) return "Driftwood In-Log Elevated Burner System";
        if (f.Contains("double glass")) return "Crystal Clear Double Glass Safety Barrier";
        if (f.Contains("reflective black back")) return "Black Glass Back That Reflects the Flame and Media";
        if (f.Contains("reflective black sides")) return "Black Glass Sides That Reflects the Flame and Media";
        if (f.Contains("rgb")) return "RGB Lighting Adds Playful Accent or Natural Glow Inside";
        if (f.Contains("summer kit")) return "Heat Removal Wired to a Switch";
        if (f.Contains("active heat flex")) return "Always Remove Heat to Remove The Heat Release";
        if (f.Contains("passive heat flex")) return "Keep it Simple Ducted Heat Release Register and Plenum";
        if (f.Contains("heat release louver")) return "Standardized Heat Release Register";
        if (f.Contains("air intake louver")) return "Standardized Air Intake Register";
        if (f.Contains("power vent")) return "Run Up to 8 90-Degree Elbows and 100' of Venting";
        if (f.Contains("reflective black interior")) return "Reflective Black Interior";
    ...
    private static bool IsStoneBallsMedia(MediaSelection media)
    {
        ...
        return combined.Contains("stone balls") ||
               combined.Contains("balls 2") ||
               combined.Contains("balls 4") ||
               combined.Contains("psbgu") ||
               combined.Contains("psbwu") ||
               combined.Contains("psbbu") ||
               combined.Contains("psbgm") ||
    ...
    private static string StoneBallsFamily(MediaSelection media)
    {
        ...
        if (combined.Contains("mixed") || combined.Contains("2 4") || combined.Contains("psbgm") || combined.Contains("psbwm") || combined.Contains("psbbm"))
            return "mixed";

        if (combined.Contains("4") || combined.Contains("u4") || combined.Contains("psbgu4") || combined.Contains("psbwu4") || combined.Contains("psbbu4"))
            return "uniform4";
    ...
    private static string StoneBallsColor(MediaSelection media)
    {
        ...
        if (combined.Contains("white") || combined.Contains("cottage") || combined.Contains("psbwu") || combined.Contains("psbwm"))
            return "white";

        if (combined.Contains("black") || combined.Contains("matte") || combined.Contains("psbbu") || combined.Contains("psbbm"))
            return "black";
    ...
    private static PriceLine BuildDriftwoodPriceLine(PriceBookWorkbook wb, FireplaceType type, string size, string model)
    {
        var smallMedia = new MediaSelection { Key = "small_driftwood", DisplayName = "Small Driftwood", IsPremium = true };
        var largeMedia = new MediaSelection { Key = "large_driftwood", DisplayName = "Large Driftwood", IsPremium = true };
        ...
        if (smallQty > 0) parts.Add($"{smallQty} Small");
        if (largeQty > 0) parts.Add($"{largeQty} Large");
    ...
    private static int CalculatePremiumMediaQuantityByGroup(PriceBookWorkbook wb, FireplaceType type, string group, string size, string model)
    {
        type = ForceOutdoorVentFreeTypeFromModel(type, model);
        var sizeNum = Digits(size);
        if (string.IsNullOrWhiteSpace(sizeNum)) return 0;
    ...
    private static bool IsRoomDefinerReflectiveFeature(string feature)
    {
        ...
        return normalized.Contains("reflective black back") ||
               normalized.Contains("reflective back") ||
               normalized.Contains("reflective black sides") ||
               normalized.Contains("reflective sides");
    }
    ...
    private static bool IsDoubleCorner(string model, string size, string glassHeight)
    {
        ...
        return combined.Contains("double corner") ||
               compact.Equals("DC", StringComparison.OrdinalIgnoreCase) ||
    ...
    {
        var normalized = Normalize(feature);
        return normalized.Contains("reflective") &&
               (normalized.Contains("side") || normalized.Contains("sides"));
    }
    ...
    private static bool IsRoomDefiner(string model, string size, string glassHeight)
    {
        ...
        return modelOnly.Equals("rd", StringComparison.OrdinalIgnoreCase) ||
               ...
               combined.Contains("room definer") ||
               combined.Contains("dvdrd") ||
               combined.Contains(" rd ") ||
               combined.EndsWith(" rd") ||
               combined.StartsWith("rd ");
    }

    private static int CalculatePremiumMediaQuantity(PriceBookWorkbook wb, FireplaceType type, MediaSelection media, string size, string model)
    {
        type = ForceOutdoorVentFreeTypeFromModel(type, model);
        ...
        if (type == FireplaceType.Traditional && (mediaKey.Contains("tr42bch") || mediaKey.Contains("tr46bch") || mediaKey.Contains("pmd birch") || mediaKey.Contains("pmdbirch") || mediaKey.Contains("loak42") || mediaKey.Contains("loak46")))
            return 1;

        if (string.IsNullOrWhiteSpace(sizeNum)) return 1;
    ...
    private static string MediaCalculationGroup(MediaSelection media)
    {
        ...
        if (combined.Contains("gold") || combined.Contains("aqua") || combined.Contains("glass")) return "fireglass";
        if (combined.Contains("diamond") || combined.Contains("rain")) return "raindrop_diamonds";
        if (combined.Contains("black stones") || combined.Contains("white stones") || combined.Contains("premium stones")) return "premium_stones";
        if (combined.Contains("grey balls 2") || combined.Contains("white balls 2") || combined.Contains("black balls 2") || combined.Contains("u2")) return "stone_uniform_2";
        if (combined.Contains("grey balls 4") || combined.Contains("white balls 4") || combined.Contains("black balls 4") || combined.Contains("u4")) return "stone_uniform_4";
        if (combined.Contains("mixed")) return "stone_mixed";
        if (combined.Contains("loak42") || combined.Contains("loak46") || combined.Contains("large oak")) return "traditional_oak";
        if (combined.Contains("tr42bch") || combined.Contains("tr46bch") || combined.Contains("pmdbirch") || combined.Contains("pmd birch")) return "traditional_birch";
        if (combined.Contains("small drift")) return "drift_small";
        if (combined.Contains("large drift")) return "drift_large";
        if (combined.Contains("driftwood") || combined.Contains("drift wood")) return "driftwood";
        if (combined.Contains("birch")) return "birchwood";
        if (combined.Contains("ember")) return "embers";
        if (combined.Contains("branch")) return "branches";
        if (combined.Contains("pebble")) return "ceramic_pebbles";
        if (combined.Contains("white log")) return "white_logs";
        if (combined.Contains("log")) return "ceramic_logs";
        return key;
    }
    private static int FindLargeMediaQuantity(PriceBookWorkbook wb, string group, string sizeNum)
    {
        ...
        foreach (var row in rows.Where(r => Normalize(First(r.RawValues, "Column1", "UNIT SIZE")).Contains(Normalize(sizeToken))))
        {
            ...
            if (group is "birchwood" or "embers" or "branches" or "drift_small" or "drift_large")
            {
                ...
                if (group == "drift_small")
                {
                    ...
                    if (small.Success && int.TryParse(small.Groups[1].Value, out var q)) return q;
                }

                if (group == "drift_large")
                {
                    var large = Regex.Match(value, @"(\d+)\s*Lrg", RegexOptions.IgnoreCase);
                    if (large.Success && int.TryParse(large.Groups[1].Value, out var q)) return q;
                }
    ...
    private static int FindOutdoorMediaQuantity(PriceBookWorkbook wb, string group, string sizeNum)
    {
        ...
        var wantsDiamonds =
            normalizedGroup.Contains("diamond") ||
            normalizedGroup.Contains("rain");

        var wantsFireGlass =
            normalizedGroup.Contains("fireglass") ||
            normalizedGroup.Contains("fire glass") ||
        ...
        var rows = wb.Rows
            .Where(r => Eq(r.SheetName, "Outdoor Media - Calculation"))
            .ToList();
        ...
        foreach (var row in rows)
        {
            ...
            var poundsText = wantsDiamonds
                ? First(raw, "DIAMOND QUANTITIES", "Diamond Quantities", "Column3")
    ...
    private static int FindIndoorMediaQuantity(PriceBookWorkbook wb, string group, string sizeNum, string styleCode)
    {
        ...
        if (string.IsNullOrWhiteSpace(blockTitle)) return 0;
        ...
        foreach (var row in rows)
        {
            ...
            if (text.Contains("For Room Definers", StringComparison.OrdinalIgnoreCase) || text.Contains("ROOM DEFINER", StringComparison.OrdinalIgnoreCase))
            ...
            if (string.IsNullOrWhiteSpace(quantityText))
                quantityText = row.RawValues.Values.Skip(1).FirstOrDefault(v => !string.IsNullOrWhiteSpace(v)) ?? string.Empty;
            ...
            if (group == "stone_mixed") return 1;
            if (group is "stone_uniform_2" or "stone_uniform_4") return 1;
    ...
    private static bool IsGlassPoundMediaGroup(string group)
    {
        ...
        return normalized.Contains("fireglass") ||
               normalized.Contains("fire glass") ||
               normalized.Contains("glass") ||
               normalized.Contains("raindrop") ||
    ...
    private static int ParseMediaQuantityToSellableSets(string group, string quantityText)
    {
        ...
        if (IsGlassPoundMediaGroup(group) &&
            (quantityText.Contains("lb", StringComparison.OrdinalIgnoreCase) ||
...
private static string IndoorBlockTitle(string group) => group switch
    ...
    private static bool IsIndoorMediaBlockHeader(string text)
    {
        ...
        return n.Contains("fireglass") ||
               n.Contains("raindrop and black diamonds") ||
               n.Contains("ceramic pebbles black white earth") ||
               n.Contains("white logs") ||
               n.Contains("premium stones") ||
               n.Contains("ceramic logs") ||
               n == "birchwood" ||
               n == "embers" ||
               n.Contains("branches small large") ||
               n.Contains("driftwood small large") ||
               n.Contains("stone balls 2 uniform") ||
               n.Contains("stone balls 4 uniform") ||
               n.Contains("stone balls 2 4 mized") ||
               n.Contains("driftwood small large");
    }
    ...
    {
        var n = Normalize(text);
        if (n.Contains("fireglass")) return "fireglass";
        if (n.Contains("raindrop and black diamonds")) return "raindrop and black diamonds";
        if (n.Contains("ceramic pebbles black white earth")) return "ceramic pebbles";
        if (n.Contains("white logs")) return "white logs";
        if (n.Contains("premium stones")) return "premium stones";
        if (n.Contains("ceramic logs")) return "ceramic logs";
        if (n == "birchwood") return "birchwood";
        if (n == "embers") return "embers";
        if (n.Contains("branches small large")) return "branches";
        if (n.Contains("driftwood small large")) return "driftwood";
        if (n.Contains("stone balls 2 uniform")) return "stone balls 2 uniform";
        if (n.Contains("stone balls 4 uniform")) return "stone balls 4 uniform";
        if (n.Contains("stone balls 2 4 mized")) return "stone balls mixed";
        return n;
    }
    ...
    private static (int Small, int Large) DriftwoodFallbackQuantities(FireplaceType type, string size, string model)
    {
        ...
        var isRoomDefiner = string.Equals(StyleCode(type, model), "RD", StringComparison.OrdinalIgnoreCase) ||
                            Normalize(model).Contains("room definer") ||
                            Normalize(model).Contains("dvdrd");
    ...
    private static int EstimatePremiumMediaQuantityFallback(string key, string size, string model)
    {
        ...
        if (key.Contains("glass", StringComparison.OrdinalIgnoreCase)) return n switch { <= 30 => 10, <= 50 => 19, <= 60 => 22, <= 70 => 26, <= 80 => 31, _ => 38 };
        if (key.Contains("stones", StringComparison.OrdinalIgnoreCase)) return n switch { <= 30 => 4, <= 50 => 7, <= 60 => 8, <= 80 => 10, _ => 12 };
        if (key.Contains("balls", StringComparison.OrdinalIgnoreCase)) return 1;
        if (key.Contains("drift", StringComparison.OrdinalIgnoreCase) || key.Contains("birch", StringComparison.OrdinalIgnoreCase)) return n switch { <= 50 => 1, <= 70 => 2, _ => 3 };
        return 1;
    }

    private static int LargeSummerKitCount(string size)
    {
        if (!int.TryParse(size, out var n)) return 2;
        if (n is 120 or 140 or 160 or 200) return 2;
        if (n is 180 or 210 or 240 or 300) return 3;
        if (n is 280 or 320 or 400) return 4;
        if (n == 340) return 5;
        return n >= 300 ? 4 : 2;
    }
    ...
    {
        var parts = new[] { input.FireplaceLocation, input.Model, string.IsNullOrWhiteSpace(input.Size) ? string.Empty : $"{Digits(input.Size)}\"", string.IsNullOrWhiteSpace(input.GlassHeight) ? string.Empty : $"{Digits(input.GlassHeight)}\" glass" }.Where(x => !string.IsNullOrWhiteSpace(x));
    ...
    private static string PassageModelCode(string? value) =>
        IsSeeThroughPassageModel(value) ? "STPASS" : "FFPASS";

    private static string PassageStyleCode(string? value) =>
        IsSeeThroughPassageModel(value) ? "ST" : "FF";
    ...
    private static string[] PassagePriceAliases(string? model)
    {
        return IsSeeThroughPassageModel(model)
            ...
            : new[] { "FFPASS", "PASSFF", "PASSFRONTFACING", "FRONTFACINGPASSAGE", "PASSAGEFRONTFACING", "PASSAGEFF" };
    }

    private static bool PassageCodeMatch(string compactText, string? model)
    {
        return PassagePriceAliases(model)
            .Any(alias => compactText.Contains(alias, StringComparison.OrdinalIgnoreCase));
    }

    private static bool IsPassageRowText(string text, string compactText, string? model)
    {
        return text.Contains("passage", StringComparison.OrdinalIgnoreCase) ||
               PassageCodeMatch(compactText, model);
    }
    ...
    private static PriceRow? FindPassageBaseRow(IEnumerable<PriceRow> rows, string model)
    {
        ...
        return Best(rows, r =>
        {
            ...
            return wantsSeeThrough
                ...
                : saysFrontFacing || (!saysSeeThrough && text.Contains("passage", StringComparison.OrdinalIgnoreCase)) || PassageCodeMatch(compact, model);
        });
    }
    ...
    private static bool PassageFeatureMatches(string text, string compact, string normalizedFeature)
    {
        var f = normalizedFeature ?? string.Empty;
                ...
                // v1.4.2 Passage Outdoor Kit feature matching
        if (IsOutdoorKitFeature(f))
            return text.Contains("outdoor kit", StringComparison.OrdinalIgnoreCase) &&
                   text.Contains("passage", StringComparison.OrdinalIgnoreCase);
if (f.Contains("double glass") || f.Contains("safety barrier"))
        ...
        if (f.Contains("reflective") && f.Contains("side"))
            return text.Contains("reflective", StringComparison.OrdinalIgnoreCase) &&
                   (text.Contains("side", StringComparison.OrdinalIgnoreCase) || text.Contains("sides", StringComparison.OrdinalIgnoreCase));
    ...
    {
                // v1.4.2 style-only OD/IO description
        if (IsIndoorOutdoorSeeThroughModelCode(model) && !IsPassageModel(model))
...
glassHeight = EffectiveGlassHeight(glassHeight, model);
        ...
        return type switch
        {
            ...
            FireplaceType.Outdoor or FireplaceType.OutdoorSeeThrough => $"Outdoor {ReadableStyle(type, model)} {Digits(size)}\"",
            FireplaceType.IndoorOutdoorSeeThrough => $"Indoor Outdoor See Through {Digits(size)}\" x {GlassInches(glassHeight)}\"",
    ...
    {
                        // v1.4.2 style-only OD/IO BuildModelNumber
        if (IsIndoorOutdoorSeeThroughModelCode(model) && !IsPassageModel(model))
        {
...
// v1.4.1 block invalid Large See Through model number
        if (IsInvalidLargeSeeThroughModel(model, size))
            return string.Empty;
if (IsPassageModel(model))
            return type == FireplaceType.IndoorOutdoorSeeThrough && IsSeeThroughPassageModel(model) ? "STPASSOD" : PassageModelCode(model);
    ...
    private static string OutdoorVentFreeScreenDescription(FireplaceType type, string model)
    {
        ...
        return style.Equals("VFF", StringComparison.OrdinalIgnoreCase)
            ? "Invisible Mesh Safety Screen"
    ...
    private static string StyleCode(FireplaceType type, string model)
    {
        ...
        // v1.3.8 manual TR style-code override
        ...
        if (compactStyleModel.Equals("TR", StringComparison.OrdinalIgnoreCase) ||
            ...
            return "TR";
if (type == FireplaceType.IndoorOutdoorSeeThrough) return "ST";
        if (type == FireplaceType.OutdoorSeeThrough) return "ST";
        if (type == FireplaceType.Outdoor) return value.Contains("see") ? "ST" : "FF";
        if (type == FireplaceType.Traditional) return "TR";
        if (value.Contains("room") || value.Contains("rd")) return "RD";
        if (value.Contains("double corner") || value.Contains("dc")) return "DC";
        if (value.Contains("left") || value.Contains("lc")) return "LC";
        if (value.Contains("right") || value.Contains("rc")) return "RC";
        if (value.Contains("see") || value.Contains("st")) return "ST";
        return "FF";
    }
...
var indoorStyle = StyleCode(type, model ?? string.Empty);
        ...
        if (type == FireplaceType.OutdoorSeeThrough || indoorStyle == "ST" || value.Contains("see") || value.Contains("st")) return "VST";
        if (indoorStyle == "LC" || value.Contains("left") || value.Contains("vlc")) return "VLC";
        if (indoorStyle == "RC" || value.Contains("right") || value.Contains("vrc")) return "VRC";
        if (indoorStyle == "DC" || value.Contains("double") || value.Contains("vdc")) return "VDC";
        return "VFF";
    }
    ...
    {
        if (type == FireplaceType.Traditional) return ["traditional"];
        if (type == FireplaceType.IndoorOutdoorSeeThrough) return ["see", "through"];
    ...
    private static string ReadableStyle(FireplaceType type, string model)
    {
        ...
        var code = StyleCode(type, model);
        return code switch { "TR" => "Traditional", "ST" => "See Through", "LC" => "Left Corner", "RC" => "Right Corner", "DC" => "Double Corner", "RD" => "Room Definer", _ => "Front Facing" };
    }
    ...
    private static string GlassSuffix(string glassHeight) => GlassInches(glassHeight) switch { "16" => "R", "24" => "H", "30" => "EH", _ => string.Empty };
    private static bool ContainsGlass(PriceRow row, string glass) => Text(row).Contains($"x{glass}", StringComparison.OrdinalIgnoreCase) || Text(row).Contains($"x {glass}", StringComparison.OrdinalIgnoreCase) || Text(row).Contains($"{glass}\"", StringComparison.OrdinalIgnoreCase);
    private static bool ContainsSize(PriceRow row, string size) => string.IsNullOrWhiteSpace(size) || Regex.IsMatch(Text(row), $@"(?<!\d){Regex.Escape(size)}(?!\d)");
    private static bool ContainsAll(string text, IEnumerable<string> parts) => parts.All(p => text.Contains(p, StringComparison.OrdinalIgnoreCase));
    private static PriceRow? Best(IEnumerable<PriceRow> rows, Func<PriceRow, bool> predicate) => rows.FirstOrDefault(predicate);
    ...
    private static string NormalizeGlassHeightAlias(string? value)
    {
        ...
        if (Regex.IsMatch(text, @"(?i)^\s*EH\s*$")) return "30";
        if (Regex.IsMatch(text, @"(?i)^\s*H\s*$")) return "24";
        if (Regex.IsMatch(text, @"(?i)^\s*R\s*$")) return "16";
    ...
    private static string Text(PriceRow r) => $"{r.PartName} {r.Description} {r.Sku} {string.Join(" ", r.RawValues.Values)}";
    ...
    private static FireplaceType DetectType(string model, string size = "")
    {
        ...
        if (IsIndoorOutdoorSeeThroughModelCode(model))
            return FireplaceType.IndoorOutdoorSeeThrough;
// v1.4.1 block invalid Large See Through type detection
        if (IsInvalidLargeSeeThroughModel(model, size))
...
if (IsPassageModel(model))
        ...
        var sizeDigits = Digits(size);
                ...
                // v1.4.1 Outdoor Vent Free compact model detection
...
if (compactModel.Equals("TR", StringComparison.OrdinalIgnoreCase) ||
        ...
        if (normalized.Contains("traditional") || normalized.Contains("dvtra") || normalized.Contains("trabon") || normalized.Contains("tra bon") || Regex.IsMatch(normalized, @"\btr\b|\btra\b|\btrad\b")) return FireplaceType.Traditional;
        if (normalized.Contains("large") || normalized.Contains("long")) return FireplaceType.Large;

        var isSeeThrough = normalized.Contains("see through") || Regex.IsMatch(normalized, @"\bst\b") || normalized.Contains("st od");
        var isIndoorOutdoor = normalized.Contains("indoor outdoor") || normalized.Contains("indooroutdoor") || normalized.Contains("st od");
        var isVentFreeOutdoor = normalized.Contains("vent free") || Regex.IsMatch(normalized, @"\bvf\b|\bvff\b|\bvst\b");
