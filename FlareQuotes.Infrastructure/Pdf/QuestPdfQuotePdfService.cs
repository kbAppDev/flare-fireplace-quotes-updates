...
public sealed class QuestPdfQuotePdfService : IQuotePdfService
{
    ...
    public Task<string> BuildQuotePdfAsync(QuoteRequest request, string outputPath, CancellationToken cancellationToken = default)
    {
        QuestPDF.Settings.License = LicenseType.Community;
        Directory.CreateDirectory(Path.GetDirectoryName(outputPath) ?? Environment.CurrentDirectory);

        if (request.Tag is not PricedQuoteResult priced)
            throw new InvalidOperationException("QuoteRequest.Tag must contain PricedQuoteResult before PDF generation.");
        ...
        Document.Create(container =>
        {
            ...
            {
                container.Page(page => RenderFireplacePage(page, request, null, logo, quoteDate, quoteNumber));
                return;
            }

            foreach (var fireplace in fireplaces)
            {
                container.Page(page => RenderFireplacePage(page, request, fireplace, logo, quoteDate, quoteNumber));
            }
    ...
    private static void RenderFireplacePage(PageDescriptor page, QuoteRequest request, PricedFireplaceQuote? fireplace, string logo, string quoteDate, string quoteNumber)
    {
                // v1.4.2 PDF hyperlink resource context
        var pricedContext = request.Tag as PricedQuoteResult;
        var resourceLinkSet = fireplace is null ? null : FindMatchingResourceLinkSet(pricedContext?.ResourceLinks, fireplace);
page.Size(PageSizes.Letter);
        ...
        page.Content().Column(col =>
        {
            ...
            col.Item().Element(c => PricingTable(c, fireplace is null ? Array.Empty<PricedFireplaceQuote>() : new[] { fireplace }, resourceLinkSet));

            if (fireplace is not null)
            {
                col.Item().PaddingTop(8).Element(c => IncludedParagraph(c, new[] { fireplace }, request));

                var optionalRows = fireplace.OptionalFeatures
                    .Select(line => new OptionalPdfRow(
                        PdfFeatureName(line.Feature),
                        line.Description,
                        line.Price,
            ...
            col.Item().PaddingTop(8).Text("The listed prices are the Manufacturer's Suggested Retail Price (MSRP), valid for 30 days, and do not include venting or installation costs.")
                .Bold()
                .FontSize(8.0f)
                .LineHeight(1.1f)
                .FontColor(TextDark);
    ...
    private static void HeaderBlock(IContainer container, string logo)
    {
        container.Column(col =>
        {
            ...
            col.Item().Row(row =>
            {
                ...
                row.ConstantItem(122).Height(40).Element(c =>
                {
                    if (File.Exists(logo))
            ...
            col.Item().AlignCenter().Text(PdfContactLine())
                .FontSize(7.7f)
                .FontColor(TextMuted);
    ...
    private static void CustomerProjectDetails(IContainer container, QuoteRequest request, string quoteDate, string quoteNumber, PricedFireplaceQuote? fireplace = null)
    {
        ...
        container.Table(table =>
        {
            ...
            table.Cell().ColumnSpan(4).Element(DetailsHeaderBox).AlignCenter().Text(projectTitle).Bold().FontSize(7.7f).AlignCenter();
    ...
    private static void PricingTable(IContainer container, IReadOnlyList<PricedFireplaceQuote> fireplaces, ResourceLinkSet? resourceLinkSet)
    {
        container.Table(table =>
        {
            table.ColumnsDefinition(c =>
            ...
            foreach (var fp in fireplaces)
            {
                ...
                BodyCellLink(table, FirstNonBlank(fp.Description, fp.BaseLine.Description), fireplaceUrl);
    ...
    private static void IncludedParagraph(IContainer container, IReadOnlyList<PricedFireplaceQuote> fireplaces, QuoteRequest request)
    {
    ...
    private static IncludedCopy IncludedText(PricedFireplaceQuote fp, QuoteRequest request)
    {
        ...
        if (IsIndoorOutdoorSeeThroughIncludedCopy(fp))
            return new IncludedCopy("Included With Every Indoor Outdoor See Through Fireplace Purchase: ", $"Power Supply, Wall Switch, Remote Control, {media}, Outdoor Kit, and *Free Shipping (*Free shipping within the continental United States only)");
return fp.Type switch
        {
                        FireplaceType.IndoorOutdoorSeeThrough =>
                new IncludedCopy("Included With Every Indoor Outdoor See Through Fireplace Purchase: ", $"Power Supply, Wall Switch, Remote Control, {media}, Outdoor Kit, and *Free Shipping (*Free shipping within the continental United States only)"),
FireplaceType.Outdoor or FireplaceType.OutdoorSeeThrough =>
                new IncludedCopy("Every Outdoor Flare Fireplace Purchase Includes: ", $"Power Supply, {media}, RGB LEDs, and *Free Shipping (*USA Only, lift-gate fees may apply)"),
            FireplaceType.Large =>
                new IncludedCopy("Included With Every Large Indoor Fireplace Purchase: ", $"Power Supply, Wall Switch, Remote Control, {media}, Double Glass, Reflective Back, and *Free Shipping (*Free shipping within the continental United States only)"),
            FireplaceType.Traditional =>
                new IncludedCopy("Included With Every Traditional Fireplace Purchase: ", $"Power Supply, Wall Switch, Remote Control, {media}, RGB LEDs, and *Free Shipping (*Free shipping within the continental United States only)"),
            _ =>
                new IncludedCopy("Included With Every Indoor Fireplace Purchase: ", IsRoomDefiner(fp) && !IsSeeThroughForReflectiveBackRules(fp)
    ...
    {
        if (fp.Type is FireplaceType.IndoorSeeThrough or FireplaceType.IndoorOutdoorSeeThrough or FireplaceType.OutdoorSeeThrough)
            return true;
    ...
    private static bool IsRoomDefiner(PricedFireplaceQuote fp)
    {
        ...
        return combined.Contains("room definer") ||
    ...
    private static void DetailsValue(TableDescriptor table, string value) =>
        table.Cell().Element(DetailsBox).AlignMiddle().Text(string.IsNullOrWhiteSpace(value) ? " " : value).FontSize(6.8f).AlignLeft();

    private static void HeaderCell(TableDescriptor table, string value) =>
        table.Cell().Element(TableHeaderBox).AlignCenter().AlignMiddle().Text(value).Bold().FontSize(6.4f).AlignCenter();

    private static void BodyCell(TableDescriptor table, string value, bool accent = false) =>
        BodyCellLink(table, value, string.Empty, accent);
    ...
    private static IContainer DetailsBox(IContainer c) =>
        c.Border(1).BorderColor(Colors.Black).PaddingVertical(.85f).PaddingHorizontal(2.5f).MinHeight(10.5f).AlignMiddle();

    private static IContainer TableHeaderBox(IContainer c) =>
        c.Border(1).BorderColor(Colors.Black).PaddingVertical(.85f).PaddingHorizontal(2).MinHeight(10.3f).AlignCenter().AlignMiddle();

    private static IContainer TableBodyBox(IContainer c) =>
        c.Border(1).BorderColor(Colors.Black).PaddingVertical(.75f).PaddingHorizontal(2).MinHeight(10.6f).AlignCenter().AlignMiddle();

    private static string FindLogoPath()
    {
        var candidates = new[]
        {
            Path.Combine(AppContext.BaseDirectory, "Assets", "header_logo-light.png"),
    ...
    {
        var normalized = new string((feature ?? string.Empty).Select(ch => char.IsLetterOrDigit(ch) ? char.ToLowerInvariant(ch) : ' ').ToArray());
        normalized = string.Join(' ', normalized.Split(' ', StringSplitOptions.RemoveEmptyEntries));

        if (normalized.Contains("summit burner")) return "Summit Burner";
        if (normalized.Contains("double glass")) return "Double Glass";
        if (normalized.Contains("reflective black back")) return "Reflective Black Back";
        if (normalized.Contains("reflective black sides")) return "Reflective Black Sides";
        if (normalized.Contains("rgb")) return "RGB LEDs";
        if (normalized.Contains("summer kit")) return "Summer Kit";
        if (normalized.Contains("active heat flex")) return "Active Heat Flex";
        if (normalized.Contains("passive heat flex")) return "Passive Heat Flex";
        if (normalized.Contains("heat release louver")) return "Heat Release Louver";
        if (normalized.Contains("air intake louver")) return "Air Intake Louver";
        if (normalized.Contains("power vent")) return "Power Vent";
        if (normalized.Contains("reflective black interior")) return "Reflective Black Interior";
    ...
    private static string DisplayDateShort(string value)
    {
        ...
        return string.IsNullOrWhiteSpace(value) ? DateTime.Now.ToString("MM/dd/yy", CultureInfo.InvariantCulture) : value;
    }
    private static ResourceLinkSet? FindMatchingResourceLinkSet(IReadOnlyList<ResourceLinkSet>? sets, PricedFireplaceQuote fp)
    {
        ...
        foreach (var set in sets)
        {
            ...
            if (candidates.Any(candidate =>
                    setKey.Equals(candidate, StringComparison.OrdinalIgnoreCase) ||
    ...
    private static string FireplacePdfUrl(ResourceLinkSet? set, PricedFireplaceQuote fp)
    {
        return FirstValidUrl(
            fp.BaseLine.Url,
            PreferredResourceUrl(set, "Product Sheet", "Product", "3-Part Spec", "Specification", "Spec", "Dimension File", "Dimensions", "Framing Guide", "Wood Framing", "Metal Framing"));
    }
    ...
    private static string FeaturePdfUrl(PriceLine line, ResourceLinkSet? set)
    {
        return FirstValidUrl(
            line.Url,
            PreferredResourceUrl(set, line.Feature, line.Description, line.Sku),
    ...
    private static string PreferredResourceUrl(ResourceLinkSet? set, params string?[] preferredLabels)
    {
        ...
        return set.Links.Values.Select(CleanPdfUrl).FirstOrDefault(value => !string.IsNullOrWhiteSpace(value)) ?? string.Empty;
    }
    ...
    private static string FeatureFallbackUrl(params string?[] values)
    {
        ...
        if (text.Contains("power vent")) return "https://flarefireplaces.com/power-vent/";
        if (text.Contains("double glass") || text.Contains("safety barrier")) return "https://flarefireplaces.com/double-glass/";
        if (text.Contains("summit")) return "https://flarefireplaces.com/summit-burner/";
        if (text.Contains("summer kit")) return "https://flarefireplaces.com/summer-kit/";
        if (text.Contains("rgb") || text.Contains("led")) return "https://flarefireplaces.com/multicolor-led-lights/";
        if (text.Contains("active heat flex")) return "https://flarefireplaces.com/active-heat-flex/";
        if (text.Contains("passive heat flex")) return "https://flarefireplaces.com/passive-heat-flex/";
        if (text.Contains("heat release") || text.Contains("air intake") || text.Contains("louver")) return "https://flarefireplaces.com/free-flow-heat-release/";
        if (text.Contains("reflective")) return "https://flarefireplaces.com/reflective-back/";
        if (text.Contains("brick traditional") || text.Contains("herringbone") || text.Contains("offset")) return "https://flarefireplaces.com/traditional-fireplace/";
        if (text.Contains("media") || text.Contains("driftwood") || text.Contains("stone") || text.Contains("balls")) return "https://flarefireplaces.com/mediaoptions/traditional-media/";
    ...
    private static string Money(decimal? value) => value.HasValue ? value.Value.ToString("C0", CultureInfo.GetCultureInfo("en-US")) : string.Empty;
    ...
    private static string PdfContactLine()
    {
        ...
        var parts = new[] { email, phone, website }
            .Where(value => !string.IsNullOrWhiteSpace(value))
            .ToArray();
    ...
    {
        return PdfSettingsValue(
            "kyle@flarefireplaces.com",
            "SalesEmail",
            "SenderEmail",
            "Email",
            "EmailAddress",
    ...
    {
        return CleanWebsiteValue(PdfSettingsValue(
            "flarefireplaces.com",
            "Website",
            "CompanyWebsite",
            "WebSite",
    ...
    private static string? FindPdfSettingsPath()
    {
        var local = Environment.GetFolderPath(Environment.SpecialFolder.LocalApplicationData);

        var candidates = new[]
        {
            Path.Combine(local, "Flare Fireplace Quotes", "settings.json"),
            Path.Combine(local, "Flare Fireplaces - Quotes", "settings.json"),
            Path.Combine(local, "Flare Fireplaces - Quotes", "v3", "settings.json"),
            Path.Combine(local, "Flare Quote Builder", "settings.json")
        };
