...
public sealed class DefaultQuoteRequestParser : IQuoteRequestParser
{
    public QuoteRequest Parse(string rawText)
    {
        ...
        request.ClientName = FindValue(rawText, "Name", "Client Name", "Customer Name", "Customer", "Client", "Contact Name");
        request.Email = Regex.Match(rawText, @"[A-Z0-9._%+\-']+@[A-Z0-9.\-]+\.[A-Z]{2,}", RegexOptions.IgnoreCase).Value;
        request.Phone = NormalizePhone(FindValue(rawText, "Phone", "Telephone", "Tel", "Cell", "Mobile"));
        if (string.IsNullOrWhiteSpace(request.Phone))
            request.Phone = NormalizePhone(Regex.Match(rawText, @"(?:\+?1[\s\-.]?)?(?:\(?\d{3}\)?[\s\-.]?)\d{3}[\s\-.]?\d{4}").Value);
        ...
        request.GlassHeight = FirstNonBlank(
            NormalizeGlassHeight(FindValue(rawText, "Glass Height", "Height")),
            ExtractGlassHeightFromModelCode(request.Model),
            ExtractGlassHeightFromModelCode(rawText));
    ...
    private static string NormalizePhone(string? value)
    {
        ...
        return digits.Length == 10 ? $"({digits[..3]}) {digits.Substring(3, 3)}-{digits[6..]}" : (value ?? string.Empty).Trim();
    }
    ...
    private static string GuessClientName(string text, string email)
    {
        ...
        {
            if (line.Contains(':') || line.Contains('@') || Regex.IsMatch(line, @"\d{3}")) continue;
            if (line.Split(' ', StringSplitOptions.RemoveEmptyEntries).Length is >= 2 and <= 4) return line;
        }
