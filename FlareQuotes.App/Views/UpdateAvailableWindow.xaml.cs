...
namespace FlareQuotes.App.Views
{
    public partial class UpdateAvailableWindow : Window
    {
        ...
        public UpdateAvailableWindow(string? latestVersion)
            : this(latestVersion, string.Empty)
        {
        }

        public UpdateAvailableWindow(object? latestVersion)
            : this(latestVersion?.ToString(), string.Empty)
        {
        }
        ...
        private void ApplyContent(string? latestVersion, string? releaseNotes)
        {
            ...
            VersionLineText.Text = string.IsNullOrWhiteSpace(version)
                ? "A new version is ready to install."
                : $"Version {version} is ready to install.";

            VersionBadgeText.Text = string.IsNullOrWhiteSpace(version)
                ? "Update"
                : $"v{version}";
